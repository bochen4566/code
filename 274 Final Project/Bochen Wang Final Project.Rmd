---
title: "Time Series Analysis of US Monthly Gas Supply(274 Final Project)"
author: "Bochen Wang"
date: "2022-12-9"
output: 
  pdf_document: null
...
We can see from the graph that is is a little bit skewed to the left from the histogram.
We can see that there is a seasonal trend for seasonal data from acf/pacf.





Because there is skewed data, I want to try to preform box-cox transformation and see how it goes:
...
The third is $\nabla_{12}\nabla_{1}X_{t}$. We can see that de-seasonalized data have the lowest variance. 
...
acf maybe lag 1, lag 2, lag 3, lag 4, lag 12 means s=12,might q = 2 or 3 or 4 need to check different q because we are considering MA part also.
...
Then I also considered the AR(1) but the results having unit roots and the residual acf/pacf plots suggest that there is a MA part
...
points((length(train)+1):(length(train)+24), pred.orig, col="coral2")
```
...
#library("TSA")
par(mfrow=c(1,2))
TSA::periodogram(train, plot = T) 
TSA::periodogram(res, plot = T) 
#abline(h=0);
#axis(1,at=c(0.01, 0.02, 0.03,0.083, 0.1))
```

We can see a period here from the graph of the training data on the left,
there exhibit no obvious seasonal and periodical trend on the residual plots.
...
Package and Functions Reference:
